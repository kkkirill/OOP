...
bool Bank::put_in_account(Person *client, int sum) {
    auto person_accounts = find_accounts(client);
    int choice = person_accounts.size() == 1 ? 0
             : choose_account(person_accounts, "Choose to which account you want put money:\n");
    if (choice == -22) {
        return false;
    }
...
int Bank::choose_account(std::list<BankAccount *> accounts, std::string text) {
    ...
    return Common::input_choice(text, 1, static_cast<const int &>(accounts.size())) - 1;
}
...
void Bank::perform_services(Person *client) {
    ...
    if (person_accounts.empty()) {
        if (Common::input_choice("You don't have account. Do you want to create it? (1/0)\n", 0, 1, true)) {
    ...
    do {
        system("cls");
        show_services();
        choice = Common::input_choice("Choose service:\n", 1, 7) - 1;
        if (choice == 0) {
            ...
            sort(client,
                 static_cast<bool>(Common::input_choice("By decendance (1/0)\n", 0, 1, true)), static_cast<bool>(Common::input_choice(
...
std::ostream &operator<<(std::ostream &os, const Bank &bank) {
    ...
    for (auto &item : bank.schedule) {
        os << Common::days[item.first] << ": " << std::put_time(&item.second.first, "%H:%M") << ", "
